...
// Update simulated measurements with random variations
static void update_measurements(chip_state_t *chip) {
  // Simulate small random variations
  chip->co2 = DEFAULT_CO2 + (rand() % 20 - 10);
  chip->temp = DEFAULT_TEMP + (rand() % 30) / 10.0;
  chip->humidity = DEFAULT_HUMIDITY + (rand() % 20 - 10);
